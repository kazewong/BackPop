...
GWTC = np.load('../data/GWTC3_LVK_posterior.npz')
...
lookback_time_func = interp1d(z_axis,lookback_time,bounds_error=False,fill_value=(0,lookback_time.max()))
...
index = i
Ndim = 7
xlabel = [r'$m_{1,{\rm ZAMS}}$',r'$m_{2,{\rm ZAMS}}$',r'$\log{Z}$','$z$',r'$\alpha$',r'$f_{\rm acc}$',r'$q_{\rm crit, 3}$']
...
fill_corner(fig,posterior_array[[0,1,4,9,5,6,7]][:,::10],Ndim,axis,style=['--','-'],fill_contours=False,lw=4,levels=[0.68,0.95],smooth2d=0.1)
...
ax2.tick_params(axis="both",direction="in",which='major',width=2.5,length=6,right=True,top=True)
ax2.tick_params(axis="both",direction="in",which='minor',width=1.5,length=3,right=True,top=True)
...
ax3.tick_params(axis="both",direction="in",which='major',width=2.5,length=6,right=True,top=True)
ax3.tick_params(axis="both",direction="in",which='minor',width=1.5,length=3,right=True,top=True)
...
ax4.tick_params(axis="both",direction="in",which='major',width=2.5,length=6,right=True,top=True)
ax4.tick_params(axis="both",direction="in",which='minor',width=1.5,length=3,right=True,top=True)
...
fig.legend(custom_lines, ['Progenitor Parameters', 'Mixed', 'Hyper-parameters'],loc=(0.72,0.35),fontsize=40,frameon=False)

fig.savefig('/mnt/home/wwong/Papers/BackPop/src/static/GW150914_corner_zoomed_lowres.png',bbox_inches='tight',dpi=100)
