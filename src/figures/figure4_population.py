...
posterior_array = []
data = h5py.File('../data/GWTC3_mcmc_processed.hdf5','r')
...
for i in range(data['coord_array'].shape[0]):
    try:
        if i != 12: # Event number 12 has significant instability in KL divergence, that it is removed from the analysis. 
            corner.hist2d(posterior_array[i,:,0][posterior_array[i,:,0]>0],posterior_array[i,:,1][posterior_array[i,:,0]>0],bins=80,smooth=0.5,label='GWTC3',plot_datapoints=False,plot_density=False,levels=[0.5],ax=ax,range=[[0,45],[0,1]],contour_kwargs={'linewidths':0,})
...
plt.savefig('/mnt/home/wwong/Papers/BackPop/src/static/GWTC3_f_acc_mass.pdf',bbox_inches='tight')