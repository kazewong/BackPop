...
\kb{Because population sythesis simulations require hundreds to thousands of cpu hours, }
\kb{training an emulator which spans a high dimensional uncertainty space remains an impossibility at present.}
...
for each individual binary system, then perform population in analyses in that space. \citet{Andrews2021} (herafter A21) used \texttt{DartBoard} \citep{Andrews2018} 
to deterine the ZAMS parameters which produce GW150914-like BBH merger based on posterior samples for GW150914 and a fixed 
...
\kb{while allowing the hyper parameters to vary across their full range of physical uncertainty}
...
space that \kb{are very likely to} produce DCO mergers. To construct a list of initial guesses, we evolve a set of 
\kb{ZAMS parameters} uniformly sampled from the \kb{initial binary} parameter space, then keep the systems that 
merge within a Hubble time.

%Ideally, we would like to have explicit control over all evolutionary parameters.
\kb{Retaining} explicit control over random variables allows us to marginalize over their contribution, 
\kb{and thus allows us to} focus on progenitor parameters and hyper-parameters.
However, in practice most population synthesis codes (including \texttt{COSMIC} which we use in this study) 
define their random variables implicitly \kb{such that} the random variables are drawn within the program instead of 
being \kb{passed as arguments.} This means the function we use to evolve the binary is not fully deterministic. % function but a stochastic function instead. 
\kb{Thus}, even if the root-finding algorithm performs perfectly, forward modelling a set of roots does not 
guarantee that the \kb{simulated population} reproduces \kb{exactly} the set of posterior samples due to 
randomness in the evolution \kb{of each binary}. To assure \kb{that} the recovered progenitor and hyper-parameters 
...
observable space, \kb{and is thus} a viable channel for that specific event. Otherwise it either means \texttt{COSMIC} 
can only explain part of the posterior or cannot explain the event at all. This is completely expected behaviour since 
\texttt{COSMIC}, \kb{and isolated binary evolution more generally}, carries its own assumptions and is expected to 
fail in reproducing a subset of the event in GWTC3. \kb{One example of this are GW events} with at least one component 
\kb{with masses above the} the pair instability supernova mass limit \kb{cite Farmer, and others}.
\kb{The reprojection could also be} subject to stochasticity in the evolution \kb{of each binary}.
\kb{To account for this}, we reproject the posterior multiple times with different random seeds and 
check whether the KL divergence varies significantly. A varying KL divergence means a particular GW event is subject 
to randomness in the evolution \kb{of the progenitor}, and extra caution should be used when interpreting the result.
...
{\kb{The spin of BHs formed in isolated binaries depends strongly on the angular 
momentum transport in massive stars which is still highly uncertain (see however Bavera+, fuller+),}}
...
obtained through the root-finding process because the formation redshift does not \kb{intrinsically} affect 
...
{\kb{Once we obtain a set of binaries which successfuly map the ZAMS parameters and hyper parameters to BBH merger masses, 
...
\kb{Yes -- but I think we need to point to Jeff's result in the intro as well.}
