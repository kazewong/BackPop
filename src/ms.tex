...
To understand the origin of gravitational wave (GW) events,
modeller 
...
Due to thae implicit definition of random variables in \texttt{COSMIC}, our evolutionary function is stochastic.
...
Since posterior samples in the observable space can have multiple roots in the evolutionary parameter space,
...
In our case, we found empirically each posterior sample may at most only correspond to a handful of roots (most of the posterior samples have a unique root.), therefore we choose to assign each weight to each root,
...
This is due to the behavior of posterior in the evolutionary parameter space are larger determined by the distribution in the observable space instead of multiple possible roots per posterior sample.
