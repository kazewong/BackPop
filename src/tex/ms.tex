...
Retaining explicit control over random variables, as done in \texttt{COMPAS} 
a binary population synthesis code \citep{Riley2022}, allows us to marginalize over
...
binary evolution in \texttt{COSMIC} most significantly for massive stars, including the common envelope
...
perform the same analysis for all events in GWTC-3. Plotted in figure
\ref{fig:GWTC-3_f_acc_mass} is the posterior density in the $m_{1,{\rm
GW}}-f_{\rm acc}$ space for most of the events in GWTC-3. Each contour represents
...
For example, event with either of the component mass larger than the lower edge of PISN gap such as GW190521 cannot be explained by \texttt{COSMIC}.
...
In this paper, our main focus is to illustrate the concept of "back-propagating" GW events posterior samples,
...
We hope this letter will motivate the community to build the next generation population synthesis codes that have the following properties:
...
Combining this work and the next generation of population synthesis codes,
